                ...
                writer.writerow(
                    {
                        ...
                        "base_scan_id": entry.get("base_scan_id", entry["scan_id"]),
...
def build_single_plane_plan(
    *,
    ...
    theta_by_line_deg: Iterable[float] | None = None,
    ...
    line_count: int = 9,
    pattern_start_angle_deg: float = 0.0,
    ...
    center, radius, lines = _circular_rays(
        ...
        start_angle_deg=float(pattern_start_angle_deg),
    )
    ...
    if int(pattern_repeats) <= 0:
        raise ValueError("pattern_repeats must be positive")
    if theta_by_line_deg is None:
        theta_by_line = None
    else:
        theta_by_line = tuple(float(value) for value in theta_by_line_deg)
        if len(theta_by_line) != len(lines):
            raise ValueError(
                f"theta_by_line_deg must contain exactly {len(lines)} values"
            )
        if not np.all(np.isfinite(theta_by_line)):
            raise ValueError("theta_by_line_deg must contain finite values")
    ...
    for line_id, line in enumerate(lines):
        ...
        selected_parameters = [
            ...
            if theta_by_line is None
            or np.isclose(
                parameter["theta_deg"],
                theta_by_line[line_id],
                atol=1e-9,
                rtol=0.0,
            )
        ]
        if not selected_parameters:
            raise ValueError(
                f"line {line_id} theta={theta_by_line[line_id]:g} deg is not "
                "present in the theta parameter pool"
            )
        for parameter_id, parameter in selected_parameters:
            entries.append(
                _make_entry(
                    scan_id=len(entries),
                    line_id=line_id,
    ...
    for entry in base_entries:
        entry["base_scan_id"] = int(entry["scan_id"])
        entry["repeat_index"] = 0
    if int(pattern_repeats) > 1:
        entries = []
        for repeat_index in range(int(pattern_repeats)):
            for source in base_entries:
                entry = copy.deepcopy(source)
                entry["scan_id"] = len(entries)
    ...
    plan = {
        ...
        "pattern_start_angle_deg": float(pattern_start_angle_deg),
        "theta_by_line_deg": (
            None if theta_by_line is None else list(theta_by_line)
        ),
        "pattern_repeat_count": int(pattern_repeats),
        "base_scan_count": len(base_entries),
        "main_scan_count": sum(
            not bool(entry["reference_pose"]) for entry in entries
        ),
        "reference_scan_count": sum(
            bool(entry["reference_pose"]) for entry in entries
        ),
