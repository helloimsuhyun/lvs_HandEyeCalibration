...
class CalibrationDataset:
    ...
    def _infer_tan_group(
        self,
        requested: int | str | None,
        accepted_motion_kinds: set[str],
        label: str,
    ) -> AcquisitionGroup:
        if requested is not None:
            group = self.group(requested)
            if group.motion_kind not in accepted_motion_kinds:
                raise ValueError(
                    f"{label} group {group.name!r} has motion_kind="
                    f"{group.motion_kind!r}"
                )
            return group
        matches = [
            group
            for group in self.groups
            if group.include_in_calibration
            and group.motion_kind in accepted_motion_kinds
        ]
        if len(matches) != 1:
            raise ValueError(
                f"expected exactly one {label} group, found {len(matches)}; "
                f"select it explicitly"
            )
        return matches[0]

    def to_tan2025_dataset(
        self,
        *,
        translation_group: int | str | None = None,
        composite_group: int | str | None = None,
    ):
        """Convert compatible groups to :class:`Tan2025Dataset`.

        Translation profiles require stable sensor-channel identities. Composite
        profiles only use a PCA line and centroid, but the portable schema keeps
        their channel identities as well.
        """
        from ..tan2025.models import Tan2025Dataset

        if self.channel_id_semantics != _STABLE_CHANNEL_SEMANTICS:
            raise ValueError(
                "Tan2025 translation recovery requires stable_sensor_channel IDs"
            )
        translation = self._infer_tan_group(
            translation_group,
            {"translation", "pure_translation"},
            "translation",
        )
        composite = self._infer_tan_group(
            composite_group,
            {"composite", "composition"},
            "composite",
        )
        if translation.group_id == composite.group_id:
            raise ValueError("translation and composite groups must be distinct")

        translation_scans = list(self.iter_group_scans(translation.group_id))
        composite_scans = list(self.iter_group_scans(composite.group_id))
        plane_ids = {
            scan.plane_id for scan in [*translation_scans, *composite_scans]
        }
        if len(plane_ids) != 1:
            raise ValueError("Tan2025 requires both motion groups on one plane")
        for scan in translation_scans:
            scan.meta["motion_kind"] = "translation"
        for scan in composite_scans:
            scan.meta["motion_kind"] = "composite"
        metadata = deepcopy(self.metadata)
        metadata.update(
            {
                "portable_acquisition_mode": self.acquisition_mode,
                "portable_source": self.source,
                "portable_profile_state": self.profile_state,
            }
        )
        return Tan2025Dataset(
            translation_scans=translation_scans,
            composite_scans=composite_scans,
            metadata=metadata,
        )

    def to_tan2025(self, **kwargs):
        """Short alias for :meth:`to_tan2025_dataset`."""
        return self.to_tan2025_dataset(**kwargs)
