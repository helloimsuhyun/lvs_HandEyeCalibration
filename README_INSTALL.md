...
Source ROS and the correct driver workspace, then run the GUI. REAL mode starts
and owns the selected official driver automatically; do not start a duplicate
driver process.
...
Use `--robot ur5e` for UR5e. Its first connection extracts and caches the
factory kinematics with the official `ur_calibration` tool. Start the UR teach
pendant's External Control program before enabling motion. The same calibration
YAML is passed to both the official driver and this project's sensor-aware
MoveGroup.

After `Connect`, real movement remains locked. **Enable real motion** sends the
current joint pose as one low-speed, zero-displacement trajectory and verifies
the action result, measured joints and TCP before any scan movement is allowed.

## Design note

The adapter intentionally sends one FollowJointTrajectory goal for each
`move_j()` call because the existing workflow already iterates over the
MoveIt-generated joint path.  That keeps the modification minimal.

Later, if desired, the workflow can be upgraded to send the complete MoveIt
trajectory in one FollowJointTrajectory goal.
